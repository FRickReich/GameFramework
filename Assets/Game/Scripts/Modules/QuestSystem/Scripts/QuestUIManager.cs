...
{
	public class QuestUIManager : Game.Base.SingletonPersistent<QuestUIManager>
	{
		...
		public void showActiveQuest(int questID)
		{
			for (int i = 0; i < availableQuests.Count; i++)
			{
				if (availableQuests[i].id == questID)
				{
					CurrentQuestText.text = availableQuests[i].title;
				}
				else
				{
					CurrentQuestText.text = "none.";
				}
			}
		}
	}
